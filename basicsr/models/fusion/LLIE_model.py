...
class LLIEModel_VI(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('color_loss'):
            self.loss_cls_dict['l_color'] = build_loss(train_opt['color_loss']).to(self.device)
    ...
    def save(self, epoch, current_iter):
        ...
        # self.opt['path']['training_states']
        # self.opt['logger']['save_checkpoint_max_num']
        # 检查self.opt['path']['models']目录下以'.pth'结尾的文件数是否大于等于self.opt['logger']['save_checkpoint_max_num']，记为save_net_exceed_flag
        # 该目录下的文件命名格式为'*_数字_*.pth'，其中数字表示文件名中第一个出现的数字，记为iter_num
        # 把文件名按照iter_num排序，若save_net_exceed_flag为True，则删除最早的文件，即iter_num最小的文件，否则不删除
        # 对于self.opt['path']['training_states']亦如此
        # 以上逻辑在base_model.py中的save_network()和save_training_state()中实现
        if hasattr(self, 'net_g_ema'):
