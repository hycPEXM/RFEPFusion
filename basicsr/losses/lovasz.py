...
def lovasz_hinge(logits, labels, per_image=True, ignore=None):
    ...
    Binary Lovasz hinge loss
      logits: [B, H, W] Tensor, logits at each pixel (between -inf and +inf)
...
def lovasz_hinge_flat(logits, labels):
    """
    Binary Lovasz hinge loss
      logits: [P] Tensor, logits at each prediction (between -inf and +inf)
...
def lovasz_softmax(probas, labels, classes='present', per_image=False, ignore=None, **kwargs):
    ...
    Multi-class Lovasz-Softmax loss
      probas: [B, C, H, W] Tensor, class probabilities at each prediction (between 0 and 1)
...
def mean(l, ignore_nan=False, empty=0):
    ...
    l = iter(l)
    if ignore_nan:
        l = filter(lambda x: not isnan(x), l)
    try:
        n = 1
        acc = next(l)
